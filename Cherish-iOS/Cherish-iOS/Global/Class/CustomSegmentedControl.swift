...
class CustomSegmentedControl: UIView {
    ...
    var textColor:UIColor = .cherishBlack
...
extension CustomSegmentedControl {
    ...
    private func configSelectorView() {
        ...
        if selectedIndex == 1 {
            let selectorPositionWhenSelector1 = frame.width/CGFloat(buttonTitles.count) * CGFloat(1)
            self.selectorView.frame.origin.x = selectorPositionWhenSelector1
        }
        else {
    ...
    private func createButton() {
        ...
        for buttonTitle in buttonTitles {
            ...
            buttons[selectedIndex].titleLabel?.font = UIFont(name: "NotoSansCJKkr-Bold", size: 16)
    }
}
